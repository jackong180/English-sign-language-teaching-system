...
if os.path.exists(qt_plugins_path):
    ...
    print(f"Set QT_PLUGIN_PATH to: {qt_plugins_path}")
else:
    # 尝试其他可能的路径
    for root, dirs, files in os.walk('.'):
        if 'plugins' in dirs and 'platforms' in os.listdir(os.path.join(root, 'plugins')):
            ...
            print(f"Set QT_PLUGIN_PATH to: {os.path.join(root, 'plugins')}")
            break
...
class SignLanguageApp(QMainWindow):
    def __init__(self):
        ...
        # 加载状态
        self.statusBar.showMessage("Ready to use mock detection")
    ...
    def update_frame(self):
        ...
        if self.detection_counter % self.detection_interval == 0:
            ...
            cv2.putText(detected_frame, f"Detected: {recognized_letter}", (10, 30), 
