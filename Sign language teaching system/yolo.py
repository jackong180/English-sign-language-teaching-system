...
class YOLO(object):
    _defaults = {
        ...
        #--------------------------------------------------------------------------#
        "model_path"        : 'd:\\waveformer-yolov8-pytorch-master - 副本\\logs_voc_full\\best_epoch_weights.pth',
    ...
    def __init__(self, **kwargs):
        ...
        # 强制使用CPU模式，避免CUDA依赖问题
        self.cuda = False
        print("Forcing CPU mode to avoid CUDA dependency issues")
        self.generate()
    ...
    def detect_image(self, image, crop = False, count = False, return_results=False):
        ...
        #---------------------------------------------------------#
        # 使用绝对路径加载字体
        font_path = getattr(self, 'font_path', 'model_data/simhei.ttf')
        # 如果字体路径是相对路径，转换为绝对路径
        ...
        for i, c in list(enumerate(top_label)):
            ...
            recognized_letters.append(predicted_class)
            # 跳过绘制识别框的代码
            # box             = top_boxes[i]
            # score           = top_conf[i]
            # 
        ...
        if return_results:
            # 如果有多个识别结果，返回第一个，否则返回空字符串
            return image, recognized_letters[0] if recognized_letters else ""
        return image
